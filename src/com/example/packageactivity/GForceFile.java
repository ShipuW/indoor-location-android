...
public class GForceFile extends GForce implements OnItemClickListener,
		OnClickListener {
	private final static String TAG = "MyFeiGeFileActivity";

	private String path = "/sdcard/室内定位/"; // 当前路径
	private String pathway = "/"; // 当前路径
	...
	public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
		...
		if (position == 0) { // 回根目录
			...
			File nowFile = new File(path);
			if (nowFile.isDirectory()) { // 若是当前路径对应的是文件，则返回
				pathway = (String) adapterList.get(position).get("path");
			}
			Log.e("path", "" + pathway);
			refreshListItems(path);
		}
	...
	public void onClick(View v) {
		...
		finish();
		// Intent myPaintIntent = new
		// Intent(GForceFile.this,SeocndActivity.class);
